                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <div className="flex flex-col space-y-1 flex-shrink-0 ml-2">
                          <Badge variant={student.type === '1-1' ? 'info' : 'success'}>
                            {student.type === '1-1' ? '1-1' : 'Group'}
                          </Badge>
                          <Badge 
                            variant={
                              student.status === 'active' ? 'success' :
                              student.status === 'paused' ? 'warning' : 'danger'
                            }
                          >
                            {student.status}
                          </Badge>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs text-gray-600 mb-2">
                        ...
                        <div>
                          <span className="font-medium">Coach:</span> {student.coach}
                        </div>
                        <div>
                          <span className="font-medium">Batch:</span> {student.batch}
                        </div>
                        <div>
                          <span className="font-medium">Rating:</span> {student.rating}
                        </div>
                        <div>
                          <span className="font-medium">Attendance:</span> 
                          <span className={`ml-1 ${student.attendance >= 90 ? 'text-green-600' : student.attendance >= 75 ? 'text-yellow-600' : 'text-red-600'}`}>
                            {student.attendance}%
                          </span>
                        </div>
                      </div>

                      <p className="text-xs text-gray-500">Joined: {new Date(student.joinedDate).toLocaleDateString('en-IN')}</p>
                    </div>
